...
class slovo //задаём класс
{
protected:
    char p_c[100000]; // массив слова
    int sm_num[100]; // массив, который считает колличество элементов 
    char p_list[100];// массив, составляющий. соответствие позиции букв и колличества букв
...
public:
    ...
    void show_sm_num(void)
    {
        ...
        cout << endl;
    }// вывод, не используем (На всякий случай)
};
...
int main()
{
    ...
    if (k == 1) {
        cout << "write : ";
        cin.ignore(numeric_limits<streamsize>::max(), '\n'); // из потока ввода в консоль убирает Enter
        cin.get(st, 100000); // Забирает всё с пробелами, считывает с клавиатуры введенные данные
        bintree p_1(st); // задали массив ст с границей 100000
        p_1.show(); // вывод, чему соответсвует каждый элемент, вывод pos_2
        p_1.shifr_m(); // Задаём чему равно shifr
        p_1.shifr_v(); // Шифр выводит
        p_1.deshifr(); // Р А З А Р Х И В А Т О Р
    };
    if (k == 2) {
        cin.ignore(numeric_limits<streamsize>::max(), '\n'); 
        cout << "write name of file: ";
        FILE* f;
        char filename[40]; // массив с названием файла
        cin.get(filename, 40); // название файла
        fopen_s(&f, filename, "w+"); //Создаём файл (w+) r-читает, например
        cout << endl;
        cout << "write : ";
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
        cin.get(st, 10000);
